...
from frequencies_map import frequencies_map_from


class FrequenciesMapTest(unittest.TestCase):
    def test_empty_list(self):
        self.assertEqual(frequencies_map_from([]), {})

    def test_single_element_list(self):
        self.assertEqual(frequencies_map_from(["a"]), {"a": 1})

    def test_several_identical_elements_list(self):
        self.assertEqual(frequencies_map_from(["a", "a", "a"]), {"a": 3})
