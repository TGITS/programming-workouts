...
def drag_cb(sender, app_data, user_data):
    ...
    print(f"[drag_cb] sender is: {sender}")
    print(f"[drag_cb] app_data is: {app_data}")
    print(f"[drag_cb] user_data is: {user_data}")


def drop_cb(sender, app_data, user_data):
    # sender is group, app_data is btn_drag
    print(f"[drop_cb] sender is: {sender}")
    print(f"[drop_cb] app_data is: {app_data}")
    print(f"[drop_cb] user_data is: {user_data}")
...
with dpg.window():
    with dpg.group(horizontal=True):
        with dpg.group(
            width=300, drop_callback=drop_cb, payload_type="int"
        ):  # user_data=??
        ...
        with dpg.group(width=300, drop_callback=drop_cb, payload_type="int"):
            ...
            dpg.add_button(label="not drag this")
            btn_drag = dpg.add_button(
                label="drag me to another group then drop", drag_callback=drag_cb
            )
