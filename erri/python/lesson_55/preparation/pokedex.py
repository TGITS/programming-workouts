...
for key, value in types_by_pokemon.items():
    for type in value:
        if type in pokemon_by_types:
            pokemon_by_types[type].append(key)
        else:
            pokemon_by_types[type] = [key]

for key, value in pokemon_by_types.items():
    print(key, f'({len(value)})',"=>", " | ".join(value))
