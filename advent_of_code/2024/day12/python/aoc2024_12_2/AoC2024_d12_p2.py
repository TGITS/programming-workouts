...
def initialize_garden_plots_to_process(map: list[str]) -> list[type[GardenPlot]]:
    ...
    for y in range(y_max):
        for x in range(x_max):
            garden_plots.append(
                GardenPlot(type=map[y][x], x=x, y=y)
            )
    return garden_plots

def find_garden_plot_neighbours(map: list[str], garden_plot: type[GardenPlot])->deque[GardenPlot]:
    y_max = len(map)
    x_max = len(map[0])
    x = garden_plot.x
    y = garden_plot.y
...
def compute_regions(map: list[str], garden_plots_to_process: deque[type[GardenPlot]], processed_garden_plots: set[type[GardenPlot]]) -> list[type[Region]]:
    regions = []
