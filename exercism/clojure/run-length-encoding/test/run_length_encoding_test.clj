...
;; Tests to check the solution for encoding that use a reducer

(deftest encode-with-reduce-empty-string
  (testing "encode an empty string"
    (is (= (rle/run-length-encode-with-reduce "") ""))))

(deftest encode-with-reduce-single-characters-without-count
  (testing "encode single characters without count"
    (is (= (rle/run-length-encode-with-reduce "XYZ") "XYZ"))))

(deftest encode-with-reduce-string-with-no-single-characters
  (testing "encode string with no single characters"
    (is (= (rle/run-length-encode-with-reduce "AABBBCCCC") "2A3B4C"))))

(deftest encode-with-reduce-string-with-single-and-mixed-characters
  (testing "encode string with single and mixed characters"
    (is (= (rle/run-length-encode-with-reduce "WWWWWWWWWWWWBWWWWWWWWWWWWBBBWWWWWWWWWWWWWWWWWWWWWWWWB") "12WB12W3B24WB"))))

(deftest encode-with-reduce-multiple-whitespace
  (testing "encode string with whitespace characters mixed in it"
    (is (= (rle/run-length-encode-with-reduce "  hsqq qww  ") "2 hs2q q2w2 "))))

(deftest encode-with-reduce-lowercase
  (testing "encode string with lowercase characters"
    (is (= (rle/run-length-encode-with-reduce "aabbbcccc") "2a3b4c"))))
